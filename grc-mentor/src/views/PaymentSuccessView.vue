                      ...
                      <div class="card border-0 shadow-sm mb-4">
                        <div class="card-body p-4">
                          ...
                          <div class="d-flex flex-column flex-sm-row gap-2 mt-3">
                            <button class="btn btn-submit" @click="printReceipt" :disabled="!payload">
                              <svg class="me-2 btn-icon" viewBox="0 0 24 24">
                                <path fill="none" stroke="currentColor" stroke-width="1.6" d="M6 9h12v7H6zM6 6h12" />
                              </svg>
                              Print / Download
                            </button>

                            <button class="btn btn-outline-secondary  rounded-pill" @click="copyTx" :disabled="!payload">
                              <svg class="me-2 btn-icon" viewBox="0 0 24 24">
                            ...
                            <router-link to="/mentorship" class="btn btn-submit ms-sm-auto d-flex align-items-center">
...
export default {
  ...
  created() {
    ...
    this.txId = 'TX-' + (Math.random().toString(36).slice(2, 10)).toUpperCase();
  ...
  methods: {
    ...
    copyTx() {
      if (!this.txId) return;
      navigator.clipboard?.writeText(this.txId).then(() => {
    ...
    goToDashboard() {
      if (this.$router) this.$router.push({ name: "Dashboard" }).catch(() => { });
      else window.location.href = "/";
    },
    contactSupport() {
      if (this.$router) this.$router.push({ name: "Contact" }).catch(() => { });
      else alert("Contact support at support@example.com");
    },
    viewInvoices() {
      if (this.$router) this.$router.push({ name: "Invoices" }).catch(() => { });
...
.payment-success {
  background: radial-gradient(800px 300px at 90% -10%, rgba(13, 110, 253, 0.04), transparent 40%), #f8f9fb;
}
...
.hero {
  align-items: center;
  gap: 1rem;
  background: linear-gradient(90deg, rgba(13, 110, 253, 0.03), rgba(32, 201, 151, 0.02));
}
...
.confetti-piece {
  ...
  animation: confettiFall 1.8s cubic-bezier(.16, .84, .49, 1) forwards;
}
